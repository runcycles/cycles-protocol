...
Return the exact HTTP status + `error` code pairs defined in `cycles-protocol-v0.yaml` §ERROR SEMANTICS — including `BUDGET_EXCEEDED` (409), `OVERDRAFT_LIMIT_EXCEEDED` (409), `IDEMPOTENCY_MISMATCH` (409), `RESERVATION_FINALIZED` (409), `RESERVATION_EXPIRED` (410), `UNIT_MISMATCH` (400), `NOT_FOUND` (404), and `DEBT_OUTSTANDING` (409). Where the deployment includes a governance plane (tenant records observable to the runtime plane), the closed-tenant guard also applies: reservation create / commit / release / extend MUST return `TENANT_CLOSED` (409) when the owning tenant's status is CLOSED — see §ERROR SEMANTICS in the runtime spec; standalone runtime deployments without tenant records have no tenant status to observe and are exempt. The v0.1.26 action-governance error codes (`ACTION_QUOTA_EXCEEDED`, `ACTION_KIND_NOT_ALLOWED`, `ACTION_KIND_DENIED`) are upcoming and listed under §Upcoming below.
