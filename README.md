...
**Spec suite:** v0.1.26 &middot; **Current conformance target:** v0.1.25 &middot; **Runtime base:** v0.1.25 &middot; **Governance base:** v0.1.25.42 (semantic_base 0.1.25.9) &middot; **API path:** `/v1` &middot; **License:** Apache 2.0
...
| **Mixed (v0.1.25)** | [`cycles-governance-admin-v0.1.25.yaml`](cycles-governance-admin-v0.1.25.yaml) | Mostly runcycles' reference admin API (implementers MAY diverge), but eight cross-plane operations, selected schemas (`Event`, `EventType`, `EventData*`, `WebhookDelivery`, `WebhookRetryPolicy`, `Permission`), and a set of cross-plane invariants (the `WEBHOOK SUBSCRIPTION INVARIANTS` — binding regardless of provisioning mechanism) are normative. See [`CONFORMANCE.md`](CONFORMANCE.md). |
...
>
> **CyclesEvidence**: the runtime base also defines an optional `cycles_evidence` reference (`CyclesEvidenceRef`) on the decide / reserve / commit / release / error responses plus `GET /v1/evidence/{id}` (`getEvidence`) and `GET /v1/.well-known/cycles-jwks.json` (`getEvidenceJwks`) — the consumer surface for tamper-evident, content-addressed, Ed25519-signed audit envelopes. The envelope itself is specified in the normative companion [`cycles-evidence-v0.2.yaml`](cycles-evidence-v0.2.yaml), which also makes signer-key authority resolution (the `did:cycles` `signer_did` form + JWK Set) normative. Additive and optional — `0.1.25` clients that ignore it are unaffected.
...
Cycles is a minimum protocol. A conformant **v0.1.25** server MUST implement 12 operations (4 core runtime reserve/commit/release/extend, plus 8 cross-plane event / webhook / auth-introspection — including `getBalances`, which the admin spec re-declares as normative), MUST honor the normative schemas and the cross-plane invariants (the `WEBHOOK SUBSCRIPTION INVARIANTS`, binding regardless of provisioning mechanism), and SHOULD implement an additional 4 runtime operations (decide, listReservations, getReservation, createEvent) that the spec marks OPTIONAL — together making up the 16-operation v0.1.25 protocol surface plus those schema + invariant contracts. Otherwise the server is free to implement tenant management, budget provisioning, key rotation, and audit UX however it likes. The additional v0.1.26 surface (action-kind registry + governance extensions, ~6 more ops) is upcoming and SHOULD-level today.
...
| 409 | `TENANT_CLOSED` | Owning tenant is CLOSED (deployments with a governance plane); blocks the persisting mutation surface — reserve/commit/release/extend and `POST /v1/events` (a persisting budget debit) — on fresh (non-replay) requests, and takes precedence over reservation-state errors. Dry-run and `/decide` instead return `decision=DENY` with `reason_code=TENANT_CLOSED` on fresh (non-replay) evaluations; same-key pre-close replays return the original stored response |
