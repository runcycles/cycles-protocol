...
# Suite-level release label used in merged-artifact titles/headers.
# Deliberately a single constant: the suite version names the release
# (README "Spec suite: v0.1.26"), it is not derivable from any one source.
SUITE_VERSION = "0.1.26"


def spec_version(path: Path) -> str:
    """
    info.version of a source spec — the single source of truth for the
    per-source composition metadata embedded in the merged artifacts.
    Read dynamically (never hard-coded) so the metadata cannot go stale
    when a source spec's revision is bumped; CI already validates each
    info.version against its changelog via scripts/validate_changelogs.py.
    """
...
def build_merged(
    ...
    # (No value counts in these comments on purpose — counts drift as the
    # base enums grow; the union is count-agnostic.)
    # ErrorCode: base runtime enum + ErrorCodeExtension's additional codes
    ...
    # EventType: base governance enum + EventTypeExtension's additional types
    union_enum_in_schema(
        merged["components"]["schemas"],
        "EventType",
        merged["components"]["schemas"],
    ...
    # Permission: base governance enum + PermissionExtension's additional permission
    union_enum_in_schema(
        merged["components"]["schemas"],
        "Permission",
        merged["components"]["schemas"],
...
def main() -> int:
    ...
    runtime_base_v = spec_version(RUNTIME_BASE)
    runtime_ext_v = spec_version(RUNTIME_EXT)
    action_kinds_v = spec_version(ACTION_KINDS)
    gov_base_v = spec_version(GOV_BASE)
    gov_ext_v = spec_version(GOV_EXT)

    # Protocol merged (runtime plane)
    print("\n[1/2] cycles-openapi-protocol-merged.yaml (runtime plane)")
    protocol = build_merged(
        title=f"Cycles Protocol — Runtime Plane (merged v{SUITE_VERSION})",
        version=SUITE_VERSION,
        description=(
            "MERGED ARTIFACT — not canonical. Generated by scripts/merge_specs.py.\n\n"
            f"Runtime plane for the Cycles Protocol v{SUITE_VERSION} spec suite. Covers:\n"
            ...
            "\n"
            "Source specs:\n"
            f"  - cycles-protocol-v0.yaml (runtime base v{runtime_base_v})\n"
            f"  - cycles-protocol-extensions-v0.1.26.yaml (runtime extension v{runtime_ext_v})\n"
            f"  - cycles-action-kinds-v0.1.26.yaml (action kinds v{action_kinds_v})\n"
            "\n"
    ...
    dump(
        ...
        header_comment=(
            ...
            "# ============================================================================\n"
            f"# This is the pre-built runtime-plane merged artifact for Cycles v{SUITE_VERSION}.\n"
            "# To regenerate: python scripts/merge_specs.py\n"
            "# Sources:\n"
            f"#   - cycles-protocol-v0.yaml (v{runtime_base_v}, runtime base)\n"
            f"#   - cycles-protocol-extensions-v0.1.26.yaml (v{runtime_ext_v}, runtime extension)\n"
            f"#   - cycles-action-kinds-v0.1.26.yaml (v{action_kinds_v}, action kind registry)\n"
    ...
    admin = build_merged(
        ...
        description=(
            "MERGED ARTIFACT — not canonical. Generated by scripts/merge_specs.py.\n\n"
            f"Admin/governance plane for the Cycles Protocol v{SUITE_VERSION} spec suite. Covers:\n"
            f"  - Tenant / budget / policy / API key / webhook management (governance base v{gov_base_v})\n"
            ...
            "\n"
            "Source specs:\n"
            f"  - cycles-governance-admin-v0.1.25.yaml (governance base v{gov_base_v})\n"
            f"  - cycles-governance-extensions-v0.1.26.yaml (governance extension v{gov_ext_v})\n"
            f"  - cycles-action-kinds-v0.1.26.yaml (action kinds v{action_kinds_v})\n"
            f"  - cycles-protocol-extensions-v0.1.26.yaml (runtime extension v{runtime_ext_v}, schemas only)\n"
            "\n"
    ...
    dump(
        ...
        header_comment=(
            ...
            "# GENERATED FILE — DO NOT EDIT\n"
            "# ============================================================================\n"
            f"# This is the pre-built admin-plane merged artifact for Cycles v{SUITE_VERSION}.\n"
            "# To regenerate: python scripts/merge_specs.py\n"
            "# Sources:\n"
            f"#   - cycles-governance-admin-v0.1.25.yaml (v{gov_base_v}, governance base)\n"
            f"#   - cycles-governance-extensions-v0.1.26.yaml (v{gov_ext_v}, governance extension)\n"
            f"#   - cycles-action-kinds-v0.1.26.yaml (v{action_kinds_v}, action kind registry)\n"
            f"#   - cycles-protocol-extensions-v0.1.26.yaml (v{runtime_ext_v}, runtime extension — schemas only)\n"
            "# Canonical sources live at the paths above. Edit source files, regenerate.\n"
            "# ============================================================================"
        ),
    )
